# Website/Lab URL Backfill Tracker

Persistent tracker for a deep-dig pass to find personal academic websites and/or lab/group pages for scholars currently missing BOTH `websiteUrl` and `labUrl`. Snapshot taken 2026-09-13T04:59:59.897Z. Total: 1145 people across 23 batches of 50.

Rules: only add a distinct HTTPS URL if one genuinely exists (do not fabricate). Not everyone has a personal or lab site — mark `[x]` done once researched (found or confirmed none exists), not just when a URL is added. Update `public/data.json`, run `npm test && npm run build && git diff --check`, then commit and push after each batch.
...
### Batch WL-04 (vp-0330 to vp-0409, 50 people)
...
### Batch WL-23 (vp-1464 to vp-1536, 45 people)
